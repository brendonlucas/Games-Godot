...
var animete_player
var active_moves = false

func _ready():
	lanterna = get_node("target/Camera/lanterna")
	#Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
	lanterna.light_energy = 0
	animete_player = get_node("AnimationPlayer")
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("esquerda") and active_moves:
		move_vec.x -= 1
		is_moving = true
	if Input.is_action_just_pressed("lanterna") and take_lanterna and active_moves:
		if !active_lanterna:
			lanterna.light_energy = 2
			active_lanterna = true
		elif active_lanterna:
			lanterna.light_energy = 0
			active_lanterna = false
